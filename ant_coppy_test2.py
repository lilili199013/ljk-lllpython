...
num_pop,num_iters,info_org=70,500,0.001
...
vane=list(map(num_to_vane,best_x))
best_x=[i+1 for i in best_x]
time_cost=time.time()-time_start 
print('运行时间:%s,蚂蚁个数:%s,迭代次数:%s,初始信息素浓度:%s.'%(time_cost,num_pop,num_iters,info_org))
print('best_x:%s'%best_x)
print('vane:%s'%vane)
print('value:%s'%best_y)